...
int main(int a, char* h[])
{
    ...
    GtkWidget *one, *two, *three, *four, *five, *six, *seven, *eight, *nine,
            *zero;
    GtkWidget *point, *leftBracket, *rightBracket, *divide, *multiply, *plus,
    ...
    int y = 1;
    GtkWidget* mas[20]
            = {seven, eight,    nine,        plus,         four,   five,  six,
               minus, one,      two,         three,        divide, sqrt,  zero,
               pow,   multiply, leftBracket, rightBracket, point,  result};
    for (int i = 0; i < 20; i++) {
        if (x > 3) {
            x = 0;
            y++;
